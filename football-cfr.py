...
payoff = cfr_axu.create_payoff(master_payoff)

# Create Information Set
information_set = cfr_axu.create_info_set(action, chance_action, payoff)
...
def CFR(h, i, t, pi_i, pi_other, init_utility):
        ...
        if i == 0:
            if payoff[h[0]][h[1]][h[2]]+init_utility >= 10:
                ...
                for T in range(itelater):
                    for I in player:
                        EU = CFR( [], I, T,  Decimal(str(1)), Decimal(str(1)), Decimal(str(payoff[h[0]][h[1]][h[2]])) )
                return EU
        else:
    ...
    if h in infoset_chance:
        ...
        for a in chance_action:
            ...
            dummy_eu.append(a)
            eu += sigma[info_index][a] * CFR( dummy_eu, i, t, pi_i, sigma[info_index][a]*pi_other )
        return eu
    ...
    for a in infoset_action[info_index]:
        if infoset_player[info_index] == i:
            ...
            Dummy_i.append(a)
            # print ("hoge")
            # print ("infoindedx:%d" % (info_index))
            # print (sigma[info_index])
            # print (Dummy_i, sigma[info_index][a]*pi_i, pi_other)
            nu_sigma_list[info_index][a] = CFR( Dummy_i, i, t, sigma[info_index][a]*pi_i, pi_other )
        else:
            ...
            # print ("infoindedx:%d" % (info_index))
            # print (sigma[info_index])
            # print (Dummy_Not_i, pi_i, sigma[info_index][a]*pi_other)
            nu_sigma_list[info_index][a] = CFR( Dummy_Not_i, i, t, pi_i, sigma[info_index][a]*pi_other )
